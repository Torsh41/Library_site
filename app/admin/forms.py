...
def validate_category_name(form, field): 
    categories = Category.query.filter_by(name=str(field.data).strip().lower()).all()
    for category in categories:
        if category.name == str(field.data).strip().lower():
            raise ValidationError('Такая категория уже имеется.')
