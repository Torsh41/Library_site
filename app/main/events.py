...
def add_post(msg):
    ...
    if msg['filename'] != '':
        post = TopicPost(body=str(msg['post_body']).strip().replace(
            "'", ""), edited=False, user=current_user, topic=topic, file=msg['screenshot'])
    else:
        post = TopicPost(body=str(msg['post_body']).strip().replace(
            "'", ""), edited=False, user=current_user, topic=topic, file=bytes(False))
